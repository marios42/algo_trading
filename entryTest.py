...
def determine_test_trades(hist_data, port_df, trade_df, date_index, trade_size, params, test_type, rand_arr):
    ...
    if test_type == "Random" or test_type == "Random_Fixed":
    ...
    if test_type == "Fixed_Life" or test_type == "Random":
        trade_df[trade_df.Open_Date == date] = trade_df[trade_df.Open_Date == date].apply(override_life, axis=1)
    elif test_type == "Fixed_Target" or test_type == "Random_Fixed":
...
def run_test(hist_data, trade_df, port_df, trade_size, params, test_type, trade_cnt):
    ...
    for date_index in range(1, len(hist_data.index)):
        [trade_df, port_df] = determine_test_trades(hist_data, port_df, trade_df, date_index, trade_size, params, test_type, rand_arr)
...
if __name__ == '__main__':
    ...
    [trade_df_rand, port_df_rand] = util.initialise_dfs(all_data, initial_capital)
    [trade_df_rand_1, port_df_rand_1] = util.initialise_dfs(all_data, initial_capital)
    ...
    # print("Run Random Entry Test")
    # [trade_df_rand, port_df_rand] = run_test(all_data, trade_df_rand, port_df_rand, trade_size, params,
    #                                          "Random", len(trade_df_life.index))
    print("Run Random Entry Test")
    [trade_df_rand_1, port_df_rand_1] = run_test(all_data, trade_df_rand, port_df_rand, trade_size, params,
                                                 "Random_Fixed", len(trade_df_life.index))
    ...
    # plt.plot((port_df_rand.Total - initial_capital) / initial_capital, color="black")
    plt.plot((port_df_rand_1.Total - initial_capital) / initial_capital, color="green")
    plt.show()